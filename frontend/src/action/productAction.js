...
rating = 0})=>async (dispatch)=>{
    ...
    try {
        const  {data} =await axios.get( `/api/products?pageNumber=${pageNumber}&name=${name}&brand=${brand}&category=${category}&min=${min}&max=${max}&rating=${rating}&order=${order}`);
        dispatch({
...
export const listProductCategories = () => async (dispatch) => {
  ...
  export const listProductBrands = ({name='', category = ''}) => async (dispatch) => {
    dispatch({
      type: PRODUCT_BRAND_LIST_REQUEST,
    });
    try {
      const { data } = await axios.get(`/api/products/brands?name=${name}&category=${category}`);
