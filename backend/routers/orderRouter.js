...
ordreRouter.get(
   ...
   expressAsyncHandler(async (req, res) => {
    ...
    const page=Number(req.query.pageNumber) || 1;
     const numOrder = req.query.numOrder || '';
     const numFilter=numOrder ? {_id: numOrder}: {};
...
ordreRouter.get('/mine',isAuth,expressAsyncHandler(async(req,res)=>{
...
ordreRouter.put('/:id/pay',isAuth,expressAsyncHandler(async(req,res)=>{
...
expressAsyncHandler(async(req,res)=>{
   ...
   if(order){
      ...
      const updatedOrder=await order.save();
      res.send({message:'Order Diliverd',order:updatedOrder});
   }else{
      res.status(404).send({message:'Order Not Found'});
   }
}))
